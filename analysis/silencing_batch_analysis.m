...
lnManip = {'ctrl', 'y', 'ts', 'd'};
saveDir{1} = '/Users/asa/Modeling/modeling_results/2020-01-20_LN_silcencing_batch';
saveDir{2} = [saveDir{1}, '_y'];
saveDir{3} = [saveDir{1}, '_ts'];
saveDir{4} = [saveDir{1}, '_d'];
...
for iParam = 1%:nParam
    for iManip = 1:nManip
        % Load model, get FR
        tmpModel = load(fullfile(saveDir{iManip} , [p(iParam).fname '.mat']));
        m(iManip) = tmpModel.m;
